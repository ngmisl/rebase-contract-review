...
contract RewardPool {
    ...
    function restake(
        address user,
        uint256 amount
    ) external onlyRewardDistributor updateReward(user) {
    ...
    function payReward(
        address user
    ) public onlyRewardDistributor updateReward(user) {
...
contract RebaseRewards is Rebased {
    ...
    constructor(address rebase) {
        ...
        for (uint i = 0; i < _tokens.length; i++) {
            _pools[_tokens[i]] = address(
                new RewardPool(quantity, rewardToken, duration)
            );
        }
    }
