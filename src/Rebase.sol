...
contract ReToken is ERC20Snapshot {
    ...
    function mint(
        address to,
        uint tokens
    ) external onlyDeployer returns (uint) {
        _mint(to, tokens);
        return totalSupply();
    }
...
contract Rebase is ERC20Burnable {
    ...
    event Stake(address indexed user, address indexed token, uint quantity);

    event Unstake(address indexed user, address indexed token, uint quantity);
    ...
    function unstake(address token, uint quantity) external {
        ...
        require(!user.locked, "No reentrancy");
        require(
            quantity > 0 && quantity <= tokenStake,
    ...
    function restake(address[] memory apps, address[] memory tokens) external {
        ...
        for (uint i = 0; i < apps.length; i++) {
            ...
            if (!user.tokenApps[token].contains(app)) {
                ...
                user.tokens.add(token);
                require(
                    userStake > 0 && _restake(app, token, userStake),
                    "Unable to restake to provided app"
                );
            }
        }
    ...
    function _updateAppStakes(
        User storage user,
        address token,
        uint addedStake,
        address[] memory addedApps
    ) internal {
    ...
    function _getReToken(address token) internal returns (ReToken) {
        ...
        if (!exists) {
            reToken = Clones.cloneDeterministic(
                _clonableToken,
                bytes32(tokenId)
            );
